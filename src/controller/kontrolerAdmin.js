...
const dashboardPage = async (req, res) => {
  try {
    ...
    return res.render('admin/dashboard', { judul: 'Admin Dashboard', stats: { totalResep, pendingCount, approvedCount } });
  } catch (err) {
...
const listResepPage = async (req, res) => {
  try {
    ...
    let semua = await Resep.find({}).sort({ tanggalDibuat: -1 }).populate('submittedBy', 'namaPengguna').lean();
    ...
    semua = semua.map(r => {
      ...
      const approvedAt = isApproved ? new Date(r.approvedAt || r.tanggalDiperbarui || r.tanggalDibuat).toLocaleString('id-ID', { day:'2-digit', month:'short', year:'numeric' }) : null;

      return {
        ...r,
        kategoriLabel: kategoriMap[r.kategori] || (r.kategori || '').replace('_', ' '),
        isApproved,
        isPending,
        isRejected,
        approvedAt
...
const listPending = async (req, res) => {
  try {
    ...
    const pending = await Resep.find({ status: 'pending' }).sort({ tanggalDibuat: -1 }).populate('submittedBy', 'namaPengguna').lean();
...
const createResep = async (req, res) => {
  try {
    ...
    const daftarBahan = typeof body.daftarBahan === 'string'
      ? body.daftarBahan.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
      : (Array.isArray(body.daftarBahan) ? body.daftarBahan : []);

    const langkah = typeof body.langkah === 'string'
      ? body.langkah.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
      : (Array.isArray(body.langkah) ? body.langkah : []);
    ...
    const r = new Resep({
      ...
      moderationBy: (body.status || 'approved') === 'approved' ? (req.session.user ? req.session.user._id : null) : undefined
    });
    await r.save();
...
const editResepPage = async (req, res) => {
  try {
    ...
    const r = await Resep.findById(req.params.id).populate('submittedBy', 'namaPengguna').lean();
    ...
    if (Array.isArray(r.daftarBahan) && r.daftarBahan.length) {
      r.daftarBahanText = r.daftarBahan.map(b => {
        if (typeof b === 'string') return b;
        const parts = [b.namaBahan || '', b.jumlah || '', b.satuan || ''].map(p => (p === undefined || p === null) ? '' : String(p));
    ...
    if (Array.isArray(r.langkah) && r.langkah.length) {
      r.langkahText = r.langkah.map(l => (typeof l === 'string') ? l : (l.deskripsi || '')).join('\n');
...
const updateResep = async (req, res) => {
  try {
    ...
    const update = {
      ...
      waktuPersiapanMenit: data.waktuPersiapan ? Number(data.waktuPersiapan) : undefined,
      waktuMemasakMenit: data.waktuMemasak ? Number(data.waktuMemasak) : undefined,
    ...
    if (typeof data.daftarBahan === 'string') {
      update.daftarBahan = data.daftarBahan.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
    } else if (Array.isArray(data.daftarBahan)) {
      update.daftarBahan = data.daftarBahan.map(s => (typeof s === 'string') ? s.trim() : s).filter(Boolean);
    }

    if (typeof data.langkah === 'string') {
      update.langkah = data.langkah.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
    } else if (Array.isArray(data.langkah)) {
      update.langkah = data.langkah.map(s => (typeof s === 'string') ? s.trim() : s).filter(Boolean);
    }
...
const approveResep = async (req, res) => {
  try {
    ...
    const updates = { status: 'approved', moderationNote: note, approvedAt: new Date(), moderationBy: req.session.user ? req.session.user._id : null };
